# LOVED GUN — SHORT FILM IMAGE-TO-VIDEO PROMPTS

- 対象：`storyboard/short-film/SHOTLIST.md`
- 用途：Seedance / ComfyUI Image-to-Video / 同等の動画生成モデル
全54制作番号（CUT045のみa/bの2生成クリップ）。各プロンプトは承認済みキーフレームを初期画像として使用する。

## Canonical identifiers

- **Hinata**: slender 17-year-old Japanese schoolboy, short softly tousled hair with a light visual mass in the approved monochrome boards, pale skin, school blazer layered over a hooded sweatshirt, straight school trousers, simple sneakers, rounded school backpack, slightly slouched posture
- **Shiori**: slender 17-year-old Japanese schoolgirl, long straight hair with full bangs, school blazer and pleated skirt with a small neck ribbon, gentle reserved smile
- **Kura**: 29-year-old Japanese man, long wavy hair gathered loosely at the back with face-framing strands, clean-shaven face, tailored three-piece suit with shirt and tie, composed upright posture; no visible earring or pinstripe pattern unless present in the approved board
- **Shadow**: faceless matte-black humanoid silhouette matching Hinata's exact proportions, moving half a beat late
- **Loved Gun**: old compact dark-metal revolver, matte cylinder, dark wood grip, restrained ornament, small engraved words “LOVED GUN”; do not add iridescent color, silver filigree, floral decoration, muzzle flash, bullet, wound, or blood unless an approved prop sheet explicitly overrides this line
- **Style**: cinematic Japanese 2D anime, clean monochrome pencil layout-board linework as the visual source of truth, restrained cel shading only after color approval, soft high-key gray reality, subtle warm/cool separation by scene, 16:9 widescreen

Board reference for these identifiers: `lp/public/images/characters/you.png`, `lp/public/images/characters/kura.png`, `lp/public/images/scenes/scene-09.png`, and `lp/public/images/scenes/scene-31.png`. When prose conflicts with those approved images, the images win.

## Global negative prompt

No Koomote, no Noh mask, no masked person, no extra protagonist, no photorealism, no 3D render, no exaggerated facial distortion, no costume change, no text overlay, no subtitle, no watermark, no logo, no handheld shake, no fast zoom, no gore, no blood, no conventional gunshot, no muzzle flash, no weapon aimed at a body, no self-harm composition.

## Generation windows for cuts longer than 10 seconds

Seedance / I2Vの1生成は5〜10秒を上限目安とする。下表の「生成」区間だけを動画生成し、残りは承認済み終端フレームのホールド、雨・光・粒子の撮影処理、音の余韻で延長する。2本目の生成を足して芝居を増やさない。

| CUT | 本尺 | 生成区間 | 延長区間 |
|---|---:|---:|---:|
| CUT014–016 | 12s | 0–8s | 8–12s：終端ホールド＋環境光 |
| CUT017–019 | 13s | 0–8s | 8–13s：終端ホールド＋音の余韻 |
| CUT020–025 | 12s | 0–8s | 8–12s：終端ホールド＋微細な光／風 |
| CUT026 | 13s | 0–8s | 8–13s：反動後ホールド＋低音余韻 |
| CUT027–030 | 11s | 0–7s | 7–11s：終端ホールド＋影の微細脈動 |
| CUT031–033 | 12s | 0–8s | 8–12s：終端ホールド＋粒子／無音 |
| CUT034–035 | 11s | 0–7s | 7–11s：終端ホールド＋呼吸／間 |
| CUT036–041 | 12s | 0–8s | 8–12s：終端ホールド＋風／室内音 |
| CUT042–044 | 13s | 0–8s | 8–13s：終端ホールド＋無音 |
| CUT046 | 14s | 0–8s | 8–14s：無傷の陽をホールド＋粒子撮影 |
| CUT047–051 | 11s | 0–7s | 7–11s：終端ホールド＋粒子／呼吸／間 |

CUT045a（6s）とCUT045b（7s）は別々に1回ずつ生成し、編集点で接続する。

## Scene 01 — 雨の交差点

### CUT001

Fixed wide establishing shot of a modern Japanese city beneath a flat gray morning sky, cinematic Japanese 2D anime with delicate pencil-like linework. Keep the skyline and composition locked. Animate only thin rain falling in three depth layers and faint reflections trembling on wet roofs. Very slow exposure lift from pale gray darkness; no characters, no camera motion.

### CUT002

Fixed long shot across a rain-soaked pedestrian crossing. Hinata, matching the approved board with short softly tousled hair, a school blazer over a hooded sweatshirt, and a rounded backpack, stands still near center while umbrella silhouettes flow around him. Keep his body and camera locked; animate only the crowd sliding past, rain streaks, and slight fabric movement. Cool desaturated palette, isolated mood.

### CUT003

Extreme close-up of the boy's pale hand holding a faded photograph of a smiling long-haired schoolgirl; handwritten Japanese words are already present on the approved keyframe. Preserve all lettering and framing exactly. Animate only his thumb tracing the writing once, a tiny paper bend, and one raindrop crossing the photo edge. No camera movement.

### CUT004

Fixed medium shot at the crossing. The pedestrian signal changes from red to green; surrounding commuters begin moving while Hinata waits half a beat, then takes one restrained step. Keep his approved short tousled hair, layered hoodie-and-blazer silhouette, background perspective, and character identity unchanged. Animate only the signal change, one step, passing silhouettes, and light rain. No pan or zoom.

### CUT005

Locked low-angle close-up of wet pavement, the boy's school shoes, and his dark reflection. His right foot completes one step but the shadow remains behind for a beat. Preserve the exact road markings and composition. Animate only the shoe movement, small water ripples, falling rain, and delayed shadow. Uneasy silence beneath natural rain.

### CUT006

Fixed low shot of the same shoes and faceless matte-black shadow. The boy steps back; the shadow begins returning slightly before his body and briefly darkens at the edges. Keep camera, pavement, and reflections locked. Animate only the controlled foot movement, half-beat timing error, and a subtle pulse through the shadow. No new forms or limbs.

### CUT007

Tight close-up of Hinata looking upward, then a locked insert of a streetlamp above him. Preserve the approved facial proportions and short tousled wet hair. Animate only his eyes lifting, a small breath, the lamp flickering twice, and a restrained burst of glass fragments falling downward. No explosion, no camera shake, no dramatic zoom.

## Scene 02 — 路地〜感情銀行

### CUT008

Fixed rear wide shot in a narrow wet alley. Hinata, in the approved hooded sweatshirt under a school blazer with a rounded backpack, walks quickly away from camera without running, shoulders slightly hunched. Keep architecture and vanishing point locked. Animate only measured footsteps, blazer hem, rain, and puddle ripples. Cool blue-gray morning light, quiet urgency, no other featured character.

### CUT009

Locked close-up of an old wooden shop sign reading 感情銀行 on the approved image. Preserve the exact Japanese lettering and wood texture. Animate only droplets gathering along the lower edge and falling one at a time, with a faint swing of the sign no more than one degree. Cool rain light, shallow depth of field.

### CUT010

Fixed long shot across the alley. Hinata, matching the approved board silhouette, stands at the far left third, staring toward the old Emotion Bank facade on the right. Maintain at least forty percent empty look space. Animate only thin rain, his breathing, and a small movement of wet bangs. No walking, no camera movement, no passersby reacting.

### CUT011

Locked close-up of the boy's pale hand approaching a dark brass doorknob. A faceless matte-black shadow matching his proportions reaches half a beat late along the floor. Preserve the door, hand, and camera position. Animate only the hand stopping before contact, the delayed shadow stopping, and one droplet sliding from his sleeve.

### CUT012

Fixed frontal shot of the old door opening inward. A narrow wedge of warm amber light expands into the cool blue-gray alley while the boy remains just outside frame edge. Keep perspective locked. Animate only the slow door movement, warm light widening, rain outside, and one gentle bell movement. No camera push and no visible masked figure.

### CUT013

Locked interior wide shot looking toward the entrance. Hinata, with the approved short tousled hair, layered hoodie-and-blazer silhouette, and backpack, crosses the threshold and pauses as the door closes behind him. Warm amber brass lighting replaces the cold rain palette. Animate only two steps, the closing door, a slight bell sway, and rain disappearing from the soundtrack impression. No camera motion.

## Scene 03 — クラとラブドガン

### CUT014

Fixed medium shot of Kura, a 29-year-old Japanese man matching the approved board: long wavy hair loosely gathered at the back, face-framing strands, clean-shaven face, tailored three-piece suit, shirt, and tie. He closes an old book and raises his eyes toward the entrance. Animate only the book closing, eyes lifting, and subtle hair settling. Warm amber interior.

### CUT015

Over-the-shoulder shot from behind Kura toward Hinata in his approved hoodie-and-blazer silhouette. Hinata remains silent and shifts only his eyes toward a glass display case. Keep the 180-degree axis, background shelves, and character positions locked. Animate only the eye movement, a shallow breath, and a faint lamp flicker. No camera movement.

### CUT016

Locked prop close-up inside a glass case: the approved compact dark-metal revolver with a matte cylinder, dark wood grip, restrained ornament, and small engraved words LOVED GUN already present. Preserve the prop-board design exactly. Animate only one warm reflection moving across the glass. Do not introduce iridescent color, a hand, or muzzle flash.

### CUT017

Extreme close-up of Kura's hand turning a small key and opening the glass case. Keep the Loved Gun, engraved lettering, matte cylinder, and hand anatomy stable. Animate only the key rotation, latch release, and a narrow warm reflection across the metal. Warm brass light, controlled movement, no camera motion.

### CUT018

Locked medium two-shot across the counter with Hinata on screen left and Kura on screen right, an object separating them. Both retain canonical clothing and proportions. Animate only restrained mouth movement for their short exchange, Hinata's slight head lift, and Kura's steady gaze. Preserve the 180-degree line and warm low-contrast bank interior.

### CUT019

Tight close-up of Kura's calm face, long wavy hair loosely gathered with face-framing strands. His gaze briefly drops toward the photograph edge in Hinata's chest pocket, then returns without smiling. Animate only the eye shift and minimal mouth movement for the final line. Keep lighting, framing, and expression restrained; hold still after speaking.

## Scene 04 — 処置と最初の一発

### CUT020

Fixed wide shot of a small white treatment room: one chair, one central table, Hinata seated left and Kura standing right, Loved Gun centered between them. Preserve large negative space and the 180-degree axis. Animate only Hinata's breathing and a barely perceptible overhead light fluctuation. No rain, no clock movement, no camera motion.

### CUT021

Extreme close-up of the Loved Gun cylinder opening in Kura's hands. One thin black light settles into an empty chamber like a soft liquid ember. Keep the revolver shape and hands stable. Animate only the cylinder rotation, black light condensing, and a tiny blue-violet reflection. No bullet, no muzzle flash, no sparks.

### CUT022

Locked close-up of Kura offering the Loved Gun while Hinata's hand remains suspended between the weapon and the faded photograph. Preserve all hand positions and prop design. Animate only Hinata's fingers tightening slightly, one tremor, and Kura's steady hand. Long pause, warm skin tones against the cold white room, no camera move.

### CUT023

Fixed side medium shot. Hinata, preserving the approved short-hair and layered schoolwear design, finally accepts the old revolver and raises it toward a blank white wall while keeping his finger outside the trigger. Kura watches from a safe angle. Animate only the controlled arm lift, slight sleeve movement, and restrained mouth movement for one warning line. No zoom.

### CUT024

Memory insert, locked extreme close-up of Shiori's slender fingers tapping a rooftop chain-link fence twice. Preserve the approved school blazer, pleated skirt, and small neck-ribbon design; orange evening backlight outlines her hand. Animate only two small fingertip taps, one strand of long straight hair crossing frame, and gentle wind in the fence. Soft film grain, no camera motion.

### CUT025

Fixed wide rooftop memory at sunset. Shiori, matching the approved board with long straight hair, full bangs, school blazer, pleated skirt, and a small neck ribbon, stands near the fence; Hinata sits several steps away. Preserve distance and negative space. Animate only Shiori's hair in wind, her minimal mouth movement, and slowly drifting clouds. Warm orange light.

### CUT026

Return to the white room, fixed medium shot. Hinata opens his eyes and fires the Loved Gun at the blank wall. His arms receive one heavy restrained recoil, but there is no muzzle flash, bullet, hole, smoke, blood, or conventional gunshot. Keep Kura still and composition locked. Animate only recoil and a brief dark pressure ripple.

## Scene 05 — 影の暴走

### CUT027

Locked low close-up of Hinata's shoes and shadow on the white floor. The faceless matte-black silhouette peels upward from the floor like a thin flexible sheet while retaining his exact proportions. Animate only the slow separation and a soft black ripple. No extra limbs, no liquid splashes, no camera movement.

### CUT028

Fixed wide two-shot of Hinata facing a faceless matte-black humanoid silhouette matching his build. The shadow finishes standing in the same pose half a beat late. Keep both figures and the white room geometry stable. Animate only the delayed posture match and faint edge breathing. Kura remains still in the background.

### CUT029

Locked close-up on Hinata's pale face, then a matching close-up on Kura without crossing the axis. Hinata realizes the figure is not Shiori; Kura answers calmly. Animate only small eye movement, restrained mouth motion, and a swallow. Keep expressions natural, no tears yet, no camera motion or dramatic effect.

### CUT030

Fixed medium shot. Hinata covers the photograph in his chest pocket and begins turning away from the shadow. The matte-black silhouette starts the same movement half a beat late, then its hand changes direction toward the photo. Animate only Hinata's turn, the delayed imitation, and the shadow hand redirecting. No camera move.

### CUT031

Locked low wide shot with a restrained 24mm sense of space. The shadow's arm reaches toward the photograph before Hinata can react; blackness spreads across the floor in thin branching veins. Preserve all room geometry. Animate only the arm extension, Hinata's single backward step, and slow floor invasion. No new creatures.

### CUT032

Fixed wide view of the treatment room as small black particles crawl upward along one wall and a stopped clock hangs above. Keep the characters, furniture, and layout unchanged. Animate only the particles moving in one direction and the final halt of the clock hand. Remove all sense of rain and ambience; no camera movement.

### CUT033

Locked side wide shot. The shadow lunges one arm toward Kura; Kura takes only half a step aside, redirects the arm with his forearm, and delivers one short controlled strike to the dark core. Animate this single compact action without acrobatics. The shadow collapses downward. Keep camera and background fixed, no impact flash.

## Scene 06 — 影を見る／屋上の記憶

### CUT034

Tight fixed close-up of Hinata staring at both pale hands, the hands that held the weapon. His approved short tousled hair hangs slightly forward and his shoulders tremble once. Animate only a small tremor, breath, and eye focus shifting from one palm to the other. Cool white-room light, no camera movement, no tears yet.

### CUT035

Fixed medium shot of Kura with long wavy hair loosely gathered at the back and his approved tailored three-piece suit, still in a guarded stance. He slowly relaxes his arms and speaks without smiling. Animate only the arms lowering, restrained mouth movement, and one downward eye shift. Warm edge light against the white room, no camera motion.

### CUT036

Locked wide shot. The faded photograph lies on the floor beside the collapsed shadow. Hinata kneels and settles at the same height as the faceless silhouette, leaving a respectful gap. Animate only his careful descent, coat folds, and the shadow's faint edge pulse. Kura stays distant and still; preserve negative space.

### CUT037

Fixed side two-shot at floor level, Hinata and the faceless shadow in profile. Hinata asks one quiet question; the shadow responds only with a small wave through its shoulders. Animate only minimal mouth motion, one breath, and the subtle shadow movement half a beat late. No face appears on the shadow.

### CUT038

Rooftop memory, fixed medium two-shot at orange sunset. Shiori with long straight hair and full bangs leans against the fence; Hinata sits apart in the approved schoolwear. Animate only hair and blazer hems in wind, restrained mouth movement for their exchange, and soft cloud drift. Maintain gentle distance and the 180-degree axis.

### CUT039

Locked close-up moving only through an internal rack focus: Shiori's gentle reserved smile, then her hand placing the faded photograph into Hinata's hand. Preserve face, hair, uniform, and hand anatomy. Animate only the small smile, the handoff, and wind-touched hair. Soft orange rim light, dissolve gently back to white.

## Scene 07 — 最後の一発

### CUT040

Top-down locked close-up of the photograph on the white floor. Hinata's pale hand rests on it; the faceless matte-black shadow hand aligns over his half a beat later without covering the image completely. Animate only the delayed overlap and a soft black pulse. Keep photo content and composition exactly unchanged.

### CUT041

Locked close-up of Kura loading one final thin black light into the Loved Gun, then offering it within the same frame. His ringed hands remain steady. Animate only the chamber rotation, black light settling, and restrained mouth motion for one sentence. No bullet, no sparks, no camera movement.

### CUT042

Fixed close-up of Hinata accepting the Loved Gun with a steady hand. Preserve his approved short tousled hair and layered hooded-sweatshirt-under-blazer design. Animate only his fingers closing around the grip, one calm breath, and a slight lift of his eyes. No tremor, no zoom.

### CUT043

Locked wide shot of the white room. The faceless shadow slowly reshapes into Shiori, a slender long-haired schoolgirl in a navy uniform, while Hinata raises the Loved Gun toward her. Keep positions and framing stable. Animate only the controlled transformation and arm lift. Shiori does not smile, speak, or approach.

### CUT044

Fixed medium shot. Hinata cannot pull the trigger and slowly lowers the gun. Shiori's form returns to the faceless matte-black silhouette, which raises one hand and points quietly toward Hinata's chest. Animate only the gun lowering, reverse transformation, and single pointing gesture. No camera motion, no melodramatic expression.

### CUT045a

Locked close-up of Kura alone in the white treatment room. He starts to extend one empty hand toward an action outside the frame, stops before reaching, then lowers his eyes. Animate only the incomplete hand movement, one restrained blink, and minimal mouth movement for the line “This will hurt.” Keep all action and Hinata outside the frame. No camera movement.

Negative prompt for this clip: gun, weapon, revolver, second person in frame, Hinata in frame, hand holding an object, self-harm, threat pose, muzzle flash, wound, blood.

### CUT045b

Locked medium close-up of Hinata alone, framed from upper chest to head. Both hands and every prop remain below and outside the frame. He looks steadily ahead and speaks two quiet lines, “I won't forget” and “I'll live with you.” After the last word, hold on his unchanged face while one deep supernatural reaction sound occurs offscreen, followed by complete silence. No visible action or effect.

Negative prompt for this clip: gun, weapon, revolver, second person in frame, Kura in frame, hands in frame, object in frame, self-harm, threat pose, muzzle flash, wound, blood.

### CUT046

Immediate cut to a fixed restrained wide shot after the offscreen sound. Hinata is already standing upright and visibly unharmed, with empty hands and no weapon anywhere in frame. The faceless shadow behind him ripples once at chest height, releases soft black particles, and begins settling toward his feet. Animate only the shadow ripple and particle descent. No reenactment of the preceding action.

Negative prompt for this clip: gun, weapon, revolver, object in hand, aiming pose, trigger action, self-harm, impact on body, muzzle flash, bullet, smoke, wound, blood, fall, second person in frame.

## Scene 08 — 消えなかった悲しみ

### CUT047

Locked low close-up as scattered black particles drift slowly downward and reassemble into a natural shadow aligned beneath Hinata's shoes. Preserve floor, lighting, and body position. Animate only the controlled particle return and a final soft edge settling. No disappearance, no extra shapes, no camera movement.

### CUT048

Fixed tight close-up of Kura, long wavy hair loosely gathered with face-framing strands. He does not smile; his gaze lowers while he waits for the answer. Animate only minimal mouth movement for one question, a slow blink, and eyes lowering. Warm side light, quiet compassion, stable face and camera.

### CUT049

Locked close-up of Hinata's pale face. He looks down at the restored shadow and slowly shakes his head once, answering that it did not disappear. Animate only one small head movement, restrained lips, and moisture gathering without a falling tear. Keep hair, blazer, framing, and cool light stable.

### CUT050

Fixed medium shot with Hinata and his aligned shadow. After a long still pause, his posture straightens only slightly and he says he can walk. Animate only a slow breath, a subtle shoulder lift, and restrained mouth movement. Do not add a smile, triumphant gesture, camera push, or bright magical light.

### CUT051

Locked hand-level wide shot. Hinata places the Loved Gun gently onto a folded dark cloth on the table; Kura's hand remains nearby but does not take it. Animate only the weapon lowering, cloth compressing slightly, and Hinata's hand releasing. Preserve the prop design, no cylinder movement, no camera motion.

## Scene 09 — 雨上がり

### CUT052

Fixed wide shot from inside the Emotion Bank toward the entrance. The door opens and Hinata steps into a rain-washed street under soft light from above. Keep warm interior and cool exterior balanced. Animate only the door, two calm steps, one bell sway, and reflections glimmering outside. No rain falling, no masked figure.

### CUT053

Low tracking medium shot moving at a very slow constant pace beside Hinata's legs. His natural shadow stays perfectly aligned while he walks, and he places the faded photograph safely into his chest pocket. Animate only measured footsteps, the pocket gesture, slight cloth movement, and puddle reflections. No delayed shadow, no shake.

### CUT054

Locked long final shot of the rain-washed Japanese street. Hinata, a small dark-navy figure, merges into an ordinary flow of pedestrians while the old Emotion Bank remains far in the background. Animate only the slow crowd flow, one passing bicycle, a child's distant movement, and soft reflected light. Hold the empty street after he recedes.

## Generation notes

- Use the approved character reference image or LoRA for every cut containing a named character.
- Generate a still keyframe first from the shot list; use this document only for the motion pass.
- Lock the seed and reference strength within each scene.
- Use optical flow or frame interpolation only after character identity and line stability pass review.
- If a model invents a mask, muzzle flash, wound, extra character, or large camera motion, reject the take rather than repairing it downstream.
